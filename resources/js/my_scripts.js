...
function loadStatsPage(){
	...
	for(row_counter = 0; row_counter < table.rows.length; row_counter++){//Outer for loop iterates over each row
			if (table.rows[row_counter].cells.length >= 3 && table.rows[row_counter].cells[3].innerHTML != "Winner"){
				var winner = table.rows[row_counter].cells[1].innerHTML;
...
function loadPlayersPage(){
	...
	for(player_index = 0;player_index < players.length; player_index++){
		string += "<a href=\"#\" onclick=\"switchPlayers("+player_index+")\">" + players[player_index].name +"</a>" +"<br />"+ '\n';
		//dropdown.insertAdjacentHTML("beforeend", string);
	}
	//dropdown.insertAdjacentHTML("beforeend", string);
